...
const Profile = () => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:3000/api/event-partner/${id}`, { withCredentials: true })
            .then(response => {
                setProfile(response.data.eventPartner)
                setVideos(response.data.eventPartner.eventItems)
            })
    }, [ id ])
