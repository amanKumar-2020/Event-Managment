...
const EventPartnerRegister = () => {
  ...
  const handleSubmit = (e) => { 
    ...
    axios.post("http://localhost:3000/api/auth/event-partner/register", {
      name:businessName,
      contactName,
      phone,
      email,
      ...
      .then(response => {
        ...
        navigate("/create-event"); // Redirect to create event page after successful registration
      })
      .catch(error => {
    ...
    <div className="auth-page-wrapper">
      <div className="auth-card" role="region" aria-labelledby="partner-register-title">
        ...
        <nav className="auth-alt-action" style={{marginTop: '-4px'}}>
          <strong style={{fontWeight:600}}>Switch:</strong> <Link to="/user/register">User</Link> • <Link to="/event-partner/register">event partner</Link>
        </nav>
        ...
        <div className="auth-alt-action">
          Already a partner? <Link to="/event-partner/login">Sign in</Link>
        </div>
      </div>
    </div>
  );
...
export default EventPartnerRegister;
